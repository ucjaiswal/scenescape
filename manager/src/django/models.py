...
class Scene(models.Model):
  ...
  def __init__(self, *args, **kwargs):
    ...
    # Flag to indicate if mesh is from generateMesh flow (already aligned by mapping service)
    self._from_generate_mesh = False
    return
  ...
  def autoAlignSceneMap(self):
    """! Rotate the glb from y-up to z-up and translate to first
      quadrant. This is only executed for direct GLB file uploads,
      not for meshes generated by the generateMesh flow (which are
      already aligned by the mapping service).
    """
